...
const getAllProfiles = async(req, res) => {
    ...
    if (header) {
        ...
        try {
            ...
            const result = await Profiles.getAllProfiles(req.params);
            if (result) {
                // logger.info('Estimation fetch successfully!');
                return res.status(200).send({ message: 'Profiles fetch successfully!', result: result, success: true });
            } else {
                ...
                try { // Verify the refresh token
                    ...
                    if (result) {
                        ...
                        return res.status(200).send({
                            message: 'Profiles fetch successfully! with a new access token!',
                            result: result,
                            success: true
                        });
                    } else {
