...
const insertEstimationStatus = async(req, res) => {
    ...
    if (header) {
        ...
        try {
            ...
            // If verification succeeds, proceed to get employees
            const result = await Costing.insertEstimationStatus(req.body);
            if (result) {
                return res.status(200).send({ message: 'Estimation status added successfully!', result: result.insertId, success: true });
            } else {
                return res.status(200).send({ error: 'Something went wrong, please check data properly', result: 0, success: false });
            }
        } catch (err) {
            if (err.name === 'TokenExpiredError') {
                ...
                try {
                    ...
                    const result = await Costing.insertEstimationStatus(req.body);
                    if (result) {
                        return res.status(200).send({ message: 'Estimation status added successfully!', result: result.insertId ? true : false, success: true });
                    } else {
                        console.log(result);
                        return res.status(200).send({ error: 'Something went wrong, please check data properly', result: false, success: false });
                    }
