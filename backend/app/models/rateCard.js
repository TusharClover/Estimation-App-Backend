...
getCostingBasedOnRateCardByEstId = async(req) => {
    var estimation_id = req.estimation_id;
    try {
        const [rows] = await pool.execute(`SELECT eps.site_id, eps.site_name, eps.profile_id, eps.profile_name, COUNT(DISTINCT eps.id) AS number_of_items, eps.estimation_id, COALESCE(SUM(w.days_count), 0) AS total_days_count, AVG(rc.margin) AS margin, COALESCE(SUM(w.days_count), 0) * COUNT(DISTINCT eps.id) * rc.rate_pd AS total_rate, COALESCE(SUM(w.days_count), 0) * COUNT(DISTINCT eps.id) * rc.cost_pd AS total_cost, rc.rate_pd * COUNT(DISTINCT eps.id) AS rate_pd, rc.cost_pd * COUNT(DISTINCT eps.id) AS cost_pd FROM estimation_proposed_schedules eps LEFT JOIN weeks w ON eps.id = w.proposed_schedule_id LEFT JOIN rate_card rc ON eps.site_id = rc.site_id AND eps.profile_id = rc.profile_id WHERE eps.estimation_id = ` + estimation_id + ` GROUP BY eps.site_id, eps.site_name, eps.profile_id, eps.profile_name, rc.rate_pd, rc.cost_pd, eps.estimation_id ORDER BY eps.profile_id;
;
;
`);
        return rows; // Return true if users successfully
